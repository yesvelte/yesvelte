<script lang="ts">
	...
	let files_1: any = $state(undefined)
	let files_2: any = $state(undefined)

	let hint_1: string = $state('')
	let hint_2: string = $state('')
	let state_1: 'invalid' | undefined = $state(undefined)
	let state_2: 'invalid' | undefined = $state(undefined)
